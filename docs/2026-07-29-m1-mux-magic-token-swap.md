...
The first half holds and is measured below. **The second half does not, and could not
have** — mux-magic has 1,401 hardcoded palette utilities that paint over the one rule the
swap tokenises. That is not a token-layer failure; it is the consumer migration the plan
already schedules for M6. Naming it now is cheaper than discovering it at M6.
...
| `--container-*` | **hazard** | `max-w-md` silently becomes `max-w-lg` |
...
**Open, needs a call:** rename our `container.*` scale, or adopt Tailwind's values for it.
...
The plan's own context section said as much — *"no `dark:` variants — the app is
permanently dark"* — so this is a claim in the milestone summary that overreached, not a
surprise about the app. A working light mode needs those 1,401 utilities migrated to
`bg-surface-*` / `text-content-*` / `bg-intent-*-solid`, which is M6.

**What M1 actually proves** is the layer beneath it: the tokens resolve, compose, and
respond to an attribute flip in a real app, and adopting them costs a dark-mode
regression of 1,144 pixels in one heading. That is the hard part, and it is done. What it
does not prove is a light mode nobody has migrated to yet.
