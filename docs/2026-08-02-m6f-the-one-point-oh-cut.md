...
**The last-consumer condition resolved here, and `lounge-tv` is not a gap in it.** The decision
says 1.0.0 waits for the last consumer and names `lounge-tv` as last in M6's fleet list. That
...
- **`lounge-tv`**, whenever Kevin wants it. Three vanilla PWAs plus a Python project; it would
