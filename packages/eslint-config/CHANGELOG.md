...
- a6ace33: `createAppConfig` exempts `*.test.tsx` and icon modules from `react/no-multi-comp`.
  ...
  - **`*.test.tsx`** — a component test that needs a wrapper declares the harness beside
    ...
    to prove a picker remounts when its value changes from outside; the harness _is_ the
    test.
