// Mirror of the React binding in `../react/useMediaQuery.ts`.
//
// Preact is a separate file rather than a `preact/compat` alias
// because `castkit/packages/slatecast` has 60 KB gz to spend and
// compat is most of that budget. There is no conformance adapter
// for this core — it has no commands to drive — so the two are
// kept honest by being a line-for-line mirror, and by
// `sourceRules.test.ts` asserting the entry stays DOM-free.
