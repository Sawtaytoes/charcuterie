...
**The link family** — `TextLink`, `ButtonLink`, and `UnstyledLink` — closes the gap that had seven repos
...
work. The first two differ in **paint, not semantics**. `UnstyledLink` owns no paint and
preserves an established app-owned treatment while still using the same router seam
