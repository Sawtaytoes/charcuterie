...
- 782a774: The markdown live preview restyles when the background parse catches up. Its inline decorations rebuilt only on edits, selection, viewport, focus and option changes, so a document whose parse finished after mount showed its last lines raw — a fenced block and a link as plain text — until the reader clicked. The table pass already compared the syntax tree; the inline pass now does too.
...
- Updated dependencies [99058d6]
  - @charcuterie/logic@2.4.0
...
- e9253a0: `Card`: a caller's own `aria-labelledby` survives when there is no `heading`.
...
- c689ba5: `CATEGORICAL_SEQUENCE` now starts on Teal and Purple, and keeps Red away from both greens.
...
- Updated dependencies [c689ba5]
  - @charcuterie/tokens@1.9.0
...
- 0057edc: A tile set walks the palette so its neighbours contrast.
...
- Updated dependencies [0057edc]
  - @charcuterie/tokens@1.8.0

## 4.2.1

### Patch Changes

- c446ec6: `Main` is the containing block for its own content, so a page stops drawing a second
  scrollbar.
...
- Updated dependencies [fa4bfd8]
  - @charcuterie/tokens@1.7.0

## 4.0.0

### Major Changes

- 87233ae: Scroll memory is automatic, and a filter no longer throws the reader to the top.
...
- c59830f: Add `TimecodeInput`, a typed `hh:mm:ss.mmm` media position field with an `isRange` mode for
  a section of a file, plus the exported `parseTimecodeInput`, `formatTimecode` and
  `clampTimecode`.
- 6710e17: `RangeSlider` — a two-thumb range control. A span picked out along a bar, with an
...
- 2ed14f2: Add an equal-width mode for segmented controls and use it for narrow Board lane selectors.

## 3.33.0

### Minor Changes

- 4bfdecb: `VirtualizedGrid` mounts nothing until its container has a box, so a grid inside a `hidden` tab panel no longer scrolls the page when it is revealed.
...
- fc9a66d: `ActionTiles` — a set of actions drawn as tiles. A bordered card carrying a name and a line
  ...
  2026-08-25 choice-tile record named under _"What this deliberately does not cover"_ — a link
...
- 37eac4c: `Tabs` takes routed items. A tab with an `href` renders as a real `<a href>` through the
...
- 3d082b0: Add `Main`'s `scrollKey`, so Back returns a list to where it was scrolled.
...
- a97c2ef: Add a horizontal row layout for Board project pools.
- e580a6e: Both markdown editors offer an **Image** toolbar action when `onUploadImage` is wired
...
- f1d3384: `Board`'s move handle now wears the gesture that can actually succeed.
...
- bc4555f: `Nav` — one navigation component, and `NavBar` is deprecated
...
- 08165e8: `ReorderList` — a list put in a different order, written once instead of four times
...
- 3342cbc: `DropRail` — every move destination at once, brought to the pointer
...
- 4b93f40: Add `NavBar` — the app's destinations across the top of a header, folding into one menu
...
- f55433d: `BoardItem` takes an `accentEdge`, so a board card can carry an identity colour that follows its corner.
...
- 9f7313e: `BoardItem.titleContent` — a board card's title can be a `ReactNode`
...
- 9f7313e: `MarkdownLine` gains `isInsideLink` — draw the marks, emit no anchor

  The third and last answer to _who owns the link here_: `href` says this component does,
  ...
  the title _and_ the body, so middle-click opens the proposal — and inside one, an anchor of
...
- 9f7313e: `MarkdownLine` — one line of markdown, drawn with its inline marks and nothing else
  ...
  _contain_ one — and an anchor inside an anchor is invalid HTML, so the browser closes the
  ...
  **One rename, and it has no consumers.** `MarkdownLine` was already the name of a _type_ —
...
- 59de539: `Combobox`: a windowed list reopens where the new element is, not where the old one was.

  Scroll a virtualized list, close it, open it again, and the panel came back with a blank
  band down the top and the rows from the _previous_ scroll position stranded below it.
...
- 2c9b307: `MarkdownEditorCodeMirror`: a visible caret, and the whole frame takes a click
  ...
  **The caret was black.** CodeMirror sets `caret-color` per light/dark class, and those
  classes track whether a _CodeMirror theme_ declared `dark: true` — not what scheme the
...
- 3ff353f: `Menu`, `Listbox` and `Combobox` take an `itemSize`, and a panel row is sized by the same
  tokens as a `Button`.
...
- a2746d0: `MarkdownView` takes `resolveUrl` — re-point a link before it becomes an `href`.
  ...
  - **The scheme guard runs first.** A URL that `safeUrls.ts` refused is not a link by the
    ...
    resurrected by one. What comes _back_ is the app's own string and is used as given.
...
- Updated dependencies [f945328]
  - @charcuterie/logic@2.3.0

## 3.17.1

### Patch Changes

- be69d9c: `Combobox`: the open seed runs on **every** open, not only the first.

  Reported against Docket, on the patch that was meant to have fixed this: _"if I close the
  combobox and open it again, it's back to the top again, not the selected item."_

  The seed hung its scroll off `activeIndex` changing. That works once. Reopen the same
  picker without choosing anything and `activeIndex` still holds the index the _last_ open
...
- 6b0a1c4: `MarkdownEditorIcons` takes an `overflow` glyph, and both markdown editors pass it to the
...
- 38e1884: `Stepper` carries each step's key into the DOM as `data-flip-key`
...
- 2ba445c: `Combobox` opens on the chosen option, not the top of the list.

  `Listbox` has always followed the APG rule — _"a reopened one resumes on the current
  value rather than the top"_, in its own words. `Combobox` seeded its highlight at index
...
- Updated dependencies [966d328]
  - @charcuterie/logic@2.2.0

## 3.14.2

### Patch Changes

- c2c01d1: Live preview: prose leading, and a code span that does not shout.
  ...
  - **A code span was drawn at the same `font-size` as the prose around it.** A monospace
    face at an equal `px` size reads _larger_: fixed advance width, taller x-height. In an
  ...
  Reported against the consuming app: _"the text is still unreadable"_, on a task whose
...
- 0f26546: Fix the crash that dropped a **whole markdown document** to raw source when any link's
...
- 8bc6efb: Add `MarkdownView` — rendered markdown with no toolbar in the DOM — and stop both markdown
  ...
  `isReadOnly` on either editor means _an editor you cannot type in_: the nine formatting
...
- eb01ac3: Add `Avatar`, and let a `BoardLane` heading be a link.
...
- 4bf405a: `CopyButton` — a button that puts a string on the clipboard and says so.
...
- 3d31590: `Card` takes an `accentEdge` — a coloured bar down the leading edge that follows the card's
  own corners.
...
- 0aa9fbd: **`BadgeButton` — a `Badge` you can press, so a pill-shaped control stops being a hand-rolled `<button className="badge …">`.**
...
- ed6c687: **`Slider` — a single-value range control, so the fleet stops choosing between a native `<input type="range">` and a hand-rolled one.**
...
- Updated dependencies [79f0124]
  - @charcuterie/logic@2.1.0

## 3.8.0

### Minor Changes

- ad0a560: `VirtualizedGrid`: `AdaptiveGrid`'s layout with only the visible rows mounted
...
- 883fb95: `Field`, `FieldGroup` and `Checkbox`: props instead of a `className`
...
- 734de2b: **`Stepper`** — an ordered sequence of steps, with what belongs to each one under it.

  Docket asked for it: its phases (_"these 3 cards have to be done in order"_) were a
...
- 902741e: `MarkdownEditorCodeMirror` renders GFM tables.
...
- 2c9483d: Story titles now place each component in a grouped sidebar — `Components/Actions`,
...
- 0f27eaf: `@charcuterie/ui/markdown-editor-codemirror` — Obsidian-style live preview, opt-in.
  ...
  **The stored value is still markdown, byte for byte.** There is no document model and no
  serialiser — live preview is decorations _over_ the text, so concealing `**` hides two
...
- a01d7a6: 1.0.0 — the API has survived contact with the fleet
  ...
  **Nothing in this bump changes an API.** It is a promise about the ones already here: from
  ...
  standing rather than a gap: the component layer does not reach a Preact consumer. `lounge-tv`
